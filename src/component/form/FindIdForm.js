    ...
    return(
        <div className="h-full w-full flex flex-col items-center justify-center">
            ...
            <div className="flex flex-col mt-5 mb-10 w-1/3">
                <p className="py-3 mb-5 font-light text-sm">가입시 등록한 이메일 입력시 <br/> 아이디의 일부를 알려드립니다.</p>
                <label for="email" className="mb-4">이메일 주소</label>
                <input
            ...
            <button className="bg-[#FFC5C5] w-full p-3 mb-3 text-lg font-medium rounded-full" onClick={findEmail}>아이디 찾기</button>
            <button className="bg-stone-300 w-[48%] p-3 rounded-full" onClick={()=>{navigate("/login")}}>로그인</button>
            <button className="bg-stone-300 w-[48%] p-3 rounded-full" onClick={()=>{navigate("/find/pwd")}}>비밀번호 찾기</button>

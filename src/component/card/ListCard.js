...
function ListCard({camp}){
    ...
    return(
        // `/camp/${camp.id}`
        <Link to="/camp/1" className="w-full flex flex-col md:flex-row mb-3">
            ...
            <div className="box-border w-full md:w-1/2 px-3 py-3 flex flex-col border border-gray-400 border-dotted rounded-xl">
                <div className="relative w-full h-2/3 flex flex-col border-b pb-3 border-gray-200">                
                    <p className="text-sm text-gray-500 mb-1">{camp.type}</p>
                    <p className="font-semibold mb-3">{camp.name} </p>
                    <p className="text-sm text-gray-500">캠핑로그 ({camp.countLog})</p>
                ...
                <div className="w-full h-1/3 flex flex-col pt-2 overflow-hidden">
                    <p className="text-sm"><span className="text-gray-400 mr-3">위치</span>{camp.location}</p>
                    <p className="text-sm"><span className="text-gray-400 mr-3">소개</span>{camp.description}</p>
                    <p className="text-sm"><span className="text-gray-400 mr-3">전화번호</span>{camp.phone}</p>
                </div>
